...
from utils.data_utils import gen_batch
...
def model_test(inputs, batch_size, n_his, n_pred, inf_mode, load_path='../../models/trained/STGCN'):
    '''
...
def save_results(inputs, batch_size, n_his, n_pred, inf_mode, save_path, load_path):
    '''
    Load and save results from model from the checkpoint.
    :param inputs: instance of class Dataset, data source for test.
    :param batch_size: int, the size of batch.
    :param n_his: int, the length of historical records for training.
    :param n_pred: int, the length of prediction.
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        y = x_test[0:len_test, step_idx + n_his, :, :]
        y_ = y_test
        x_stats = x_stats
        ...
        def evaluation(y, y_, x_stats):
            ...
            if dim == 3:
                ...
                np.save(f'{save_path}/actual.npy', v)
                np.save(f'{save_path}/predicted.npy', v_)
                print("==> Complete")
                exit(0)
            else:
                # multi_step case
